...
import { ArrowUpRight, Shield, Users, AlertTriangle, Activity, ChevronRight, Wallet } from "lucide-react";
import { PortfolioDistributionChart, RiskDistributionChart } from "@/components/dashboard/charts";
...
const alerts = [
  {
    ...
    description: "StakeWise operator risk score increased from 45% to 55% in the last 24 hours.",
    time: "2 hours ago",
    color: "text-[#FF3B30]",
    bg: "bg-[#460808]",
  ...
  },
  {
    ...
    description: "Consider rebalancing: 45% of your portfolio is delegated to a single operator.",
    time: "5 hours ago",
    color: "text-[#155DFC]",
    bg: "bg-[#155DFC1A]",
  ...
  },
  {
    ...
    description: "A new highly-rated operator with 33% risk score is now accepting delegations.",
    time: "1 day ago",
    color: "text-[#A1A1AA]",
    bg: "bg-[#27272A40]",
  ...
  },
  {
    ...
    description: "Rocket Pool showed increased (12% → 18%) delegation volatility.",
    time: "2 days ago",
    color: "text-[#FE9A00]",
    bg: "bg-[#461901]",
    ...
    <div className="min-h-screen bg-background text-foreground p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Here's your portfolio overview and latest updates</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, index) => (
            ...
            >
              ...
              {stat.change && (
                <div className={`text-xs px-2 py-1 rounded-full inline-block ${stat.bg} ${stat.color}`}>
                  Low Risk <span className="ml-1">↘ {stat.change}</span>
                </div>
              )}
              {stat.subValue && <div className="text-sm text-muted-foreground">{stat.subValue}</div>}
              {stat.action && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div
            ...
            className="p-6 rounded-xl bg-card border border-border"
          >
            <h3 className="text-lg font-semibold mb-6">Portfolio Distribution by Operator</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="space-y-3">
              {alerts.map((alert, index) => (
                ...
                >
                  <AlertTriangle size={20} className={`shrink-0 mt-1 ${alert.color}`} />
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-muted-foreground mb-2">Select Next Operator</label>
              ...
              <div>
                <label className="block text-sm text-muted-foreground mb-2">Amount to Delegate (ETH)</label>
                <input
                  type="text"
                  placeholder="0.00"
        ...
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          <div className="p-6 border-b border-border flex justify-between items-center">
            ...
            <Link href="/operator" className="text-sm text-muted-foreground hover:text-white flex items-center gap-1 transition-colors">
              ...
              <div key={index} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center overflow-hidden relative">
                    <Image src={op.icon} alt={op.name} fill className="object-contain" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium">{op.name}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                        op.riskLevel === "Low" ? "bg-[#032E15] text-[#00C950]" : "bg-[#461901] text-[#FE9A00]"
                      }`}>
                        {op.riskLevel}
                      </span>
                    </div>
                    <div className="text-xs text-muted-foreground">ID: {op.id} • {op.avs}</div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Risk Score: <span className={Number(op.riskScore.replace('%', '')) < 50 ? "text-[#00C950]" : "text-[#FE9A00]"}>{op.riskScore}</span>
                  </div>
                </div>
